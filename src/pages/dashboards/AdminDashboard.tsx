              ...
              <>
                <div className="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-6 gap-6">
                  <StatCard
                    ...
                    change={stats.evolutionTotal ?? 0}
                    color="from-purple-500/20 to-purple-600/20 border-purple-500/30"
                    loading={loading}
                    onClick={() => handleStatCardClick('Total Courriers')}
                  />
                  <StatCard
                    icon={Clock}
                    title="En attente"
                    value={stats.courriersEnAttente}
                    change={stats.evolutionEnAttente ?? 0}
                  ...
                  <StatCard
                    icon={CheckCircle}
                    title="Traités"
                    value={stats.courriersTraites}
                    change={stats.evolutionTraites ?? 0}
                ...
                <StatistiquesAvancees periode={period} />
                    ...
                    <div className="space-y-4">
                      <div>
                        <div className="flex items-center justify-between mb-2">
                          ...
                          <span className="text-sm font-medium text-white">{stats.courriersEntrants}</span>
                        </div>
                        <div className="h-3 bg-slate-700 rounded-full overflow-hidden">
                          <div 
                            className="h-full bg-gradient-to-r from-purple-500 to-pink-500 transition-all duration-500"
                            style={{ 
                              width: `${(stats.courriersEntrants / Math.max(stats.totalCourriers, 1)) * 100}%` 
                            }}
                          ></div>
                        </div>
                        <div className="text-xs text-slate-500 mt-1">
                          {((stats.courriersEntrants / Math.max(stats.totalCourriers, 1)) * 100).toFixed(1)}% du total
                        </div>
                      </div>
                      <div>
                        <div className="flex items-center justify-between mb-2">
                          <span className="text-sm text-slate-400">Courriers Sortants</span>
                          <span className="text-sm font-medium text-white">{stats.courriersSortants}</span>
                        </div>
                        <div className="h-3 bg-slate-700 rounded-full overflow-hidden">
                          <div 
                            className="h-full bg-gradient-to-r from-blue-500 to-cyan-500 transition-all duration-500"
                            style={{ 
                              width: `${(stats.courriersSortants / Math.max(stats.totalCourriers, 1)) * 100}%` 
                            }}
                          ></div>
                        </div>
                        <div className="text-xs text-slate-500 mt-1">
                          {((stats.courriersSortants / Math.max(stats.totalCourriers, 1)) * 100).toFixed(1)}% du total
                        </div>
                      </div>
                ...
                <div className="bg-slate-800/50 backdrop-blur-sm border border-slate-700 rounded-xl p-6">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                    ...
                    >
                      <BarChart3 className="w-6 h-6 mx-auto mb-2 group-hover:scale-110 transition-transform" />
                      <span className="text-sm font-medium">Rapports</span>
                      <p className="text-xs text-slate-400 mt-1">Générer des statistiques</p>
                    </button>
